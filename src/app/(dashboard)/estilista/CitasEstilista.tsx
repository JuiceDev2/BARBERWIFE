...
import { formatPeso, ESTADO_COLORS, ESTADO_LABELS, ESTADO_CITA_FLUJO_LABELS, ESTADO_CITA_FLUJO_COLORS, cn } from '@/lib/utils'
...
interface Props {
  ...
  completadas: Cita[]
  turnoActivo: boolean
  turnoId?: string | null // ✅ CAMBIO: Recibimos el ID del turno
  estilistaId: string
  salonId: string
}

export default function CitasEstilista({ 
  citas, 
  completadas, 
  turnoActivo, 
  turnoId, 
  ...
  // Redirige a la caja (/cobrar/[turnoId]) para registrar el pago del servicio
  const irACobrar = () => {
    if (!turnoId) {
      toast.error('No hay un turno activo. El administrador debe abrir el local.')
      return
    }
    router.push(`/cobrar/${turnoId}`)
  }
    ...
    <div className="space-y-8">
      ...
      {citas.length === 0 ? (
        ...
        <div className="space-y-3">
          {citas.map(cita => {
            const esEspecial = cita.tipo_cita === 'especial'
            const especialListo = !esEspecial || cita.estado_cita === 'confirmada' || cita.estado_cita === 'en_proceso'

            return (
              <div key={cita.id} className="card flex items-center justify-between gap-4"
                   style={esEspecial ? { borderColor: 'var(--color-salon-300)', background: 'var(--color-salon-50)' } : undefined}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    {esEspecial && (
                      <p className="text-xs font-semibold flex items-center gap-1.5 mb-0.5" style={{ color: 'var(--color-salon-700)' }}>
                        <span title="Servicio Especial">✨ Servicio Especial</span>
                      </p>
                    )}
                    ...
                    {esEspecial && (
                      <p className="text-xs mt-1" style={{ color: 'var(--color-salon-700)' }}>
                        📍 {cita.municipio?.nombre ?? 'Sin municipio'}
                        {cita.calle && ` · ${cita.calle} ${cita.numero_exterior ?? ''}${cita.numero_interior ? ` Int. ${cita.numero_interior}` : ''}`}
                      </p>
                    )}
                    {esEspecial && cita.referencias && (
                      <p className="text-xs italic" style={{ color: 'var(--color-warm-500)' }}>
                        {cita.referencias}
                      </p>
                    )}
                    {cita.notas && (
                ...
                <div className="flex items-center gap-2">
                  {esEspecial && (
                    <span className={cn('badge', ESTADO_CITA_FLUJO_COLORS[cita.estado_cita])}>
                      {ESTADO_CITA_FLUJO_LABELS[cita.estado_cita] ?? cita.estado_cita}
                    </span>
                  )}
                  ...
                  {especialListo ? (
                    ...
                    <span className="text-xs" style={{ color: 'var(--color-warm-500)' }}>
