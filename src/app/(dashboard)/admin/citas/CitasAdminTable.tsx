...
export default function CitasAdminTable({ citas, estilistas, servicios, salonId }: Props) {
  ...
  // El cliente ya no elige estilista en el wizard: se asigna aquí.
  async function asignarEstilista(cita: Cita, estilistaId: string) {
    setAsignandoId(cita.id)
    const { error } = await supabase
      .from('citas')
      .update({ estilista_id: estilistaId || null })
      .eq('id', cita.id)

    setAsignandoId(null)
    if (error) { toast.error('Error al asignar estilista'); return }
    toast.success(estilistaId ? 'Estilista asignado' : 'Asignación removida')
    router.refresh()
  }
              ...
              <tr key={c.id} className={cn(c.estado === 'cancelada' && 'opacity-50')}>
                ...
                <td className="px-4 py-3">
                  <p className="font-medium flex items-center gap-1.5">
                    {c.tipo_cita === 'especial' && <span title="Servicio Especial">✨</span>}
                    {c.cliente?.nombre}
                  </p>
                  <p className="text-xs" style={{ color: 'var(--color-warm-500)' }}>{c.cliente?.telefono}</p>
                  {c.tipo_cita === 'especial' && c.municipio?.nombre && (
                    <p className="text-xs mt-0.5" style={{ color: 'var(--color-salon-600)' }}>
                      📍 {c.municipio.nombre}
                    </p>
                  )}
                </td>
                ...
                <td className="px-4 py-3">
                  <select
                    ...
                    value={c.estilista_id ?? ''}
                    disabled={asignandoId === c.id}
                    onChange={e => asignarEstilista(c, e.target.value)}
                  >
