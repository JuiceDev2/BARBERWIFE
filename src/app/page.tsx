...
import { getFaqData } from '@/lib/chatbot/faq-data'
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-salon-50)' }}>
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b-2"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-warm-900" style={{ color: 'var(--color-salon-800)' }}>
            {salon?.nombre ?? 'Estética Unisex Alejandra'}
          </span>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-20 text-center">
        ...
        <div className="w-16 h-0.5 mb-6" style={{ background: 'var(--color-gold-500)' }} />
      ...
      <footer className="py-6 text-center text-xs" style={{ color: 'var(--color-warm-500)' }}>
        ...
        <p className="mt-1">© {new Date().getFullYear()} {salon?.nombre ?? 'Estética Unisex Alejandra'}</p>
      </footer>

      {/* ── Chatbot de preguntas frecuentes (sin IA) ──────────── */}
      <ChatbotFAQ
        nombreSalon={salon?.nombre ?? 'Estética Unisex Alejandra'}
        telefono={salon?.telefono}
        faqData={getFaqData(salon, servicios)}
      />
    </div>
  )
}
